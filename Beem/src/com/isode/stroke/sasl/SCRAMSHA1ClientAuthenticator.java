...
public class SCRAMSHA1ClientAuthenticator extends ClientAuthenticator {
	...
	public ByteArray getResponse() {
		if (step.equals(Step.Initial)) {
			return ByteArray
					.plus(getGS2Header(), getInitialBareClientMessage());
		} else if (step.equals(Step.Proof)) {
			ByteArray clientKey = HMACSHA1.getResult(saltedPassword,
					new ByteArray("Client Key"));
			ByteArray storedKey = SHA1.getHash(clientKey);
			ByteArray clientSignature = HMACSHA1.getResult(storedKey,
					authMessage);
			...
			ByteArray result = getFinalMessageWithoutProof().append(",p=")
	...
	public boolean setChallenge(ByteArray challenge) {
		if (step.equals(Step.Initial)) {
			...
			Map<Character, String> keys = parseMap(initialServerMessage
					.toString());
			...
			String receivedClientNonce = clientServerNonce.substring(0,
					clientnonce.length());
			if (!receivedClientNonce.equals(clientnonce)) {
				return false;
			}
			serverNonce = new ByteArray(clientServerNonce.substring(clientnonce
					.length()));
			...
			saltedPassword = PBKDF2.encode(new ByteArray(
					SASLPrep(getPassword())), salt, iterations);
			authMessage = getInitialBareClientMessage().append(",")
					.append(initialServerMessage).append(",")
					.append(getFinalMessageWithoutProof());
			ByteArray serverKey = HMACSHA1.getResult(saltedPassword,
					new ByteArray("Server Key"));
			serverSignature = HMACSHA1.getResult(serverKey, authMessage);

			step = Step.Proof;
			return true;
		} else if (step.equals(step.Proof)) {
			ByteArray result = new ByteArray("v=").append(new ByteArray(Base64
	...
	private ByteArray getInitialBareClientMessage() {
		...
		return new ByteArray("n=" + escape(authenticationID) + ",r="
				+ clientnonce);
	}
	...
	private ByteArray getGS2Header() {
		...
		return new ByteArray()
				.append(channelBindingHeader)
				.append(",")
				.append(getAuthorizationID().isEmpty() ? new ByteArray()
						: new ByteArray("a=" + escape(getAuthorizationID())))
				.append(",");
	}
	...
	private ByteArray getFinalMessageWithoutProof() {
		...
		return new ByteArray("c="
				+ Base64.encode(new ByteArray(getGS2Header())
						.append(channelBindData)) + ",r=" + clientnonce)
				.append(serverNonce);
	}
