...
/**
 * A PacketExtensionProvider to parse the Avatar metadata. XML namespace
 * urn:xmpp:avatar:metadata
 */
public class AvatarMetadataProvider implements PacketExtensionProvider {

	/**
	 * Creates a new AvatarMetadataProvider. ProviderManager requires that every
			...
			if (eventType == XmlPullParser.START_TAG) {
				if ("info".equals(parser.getName())) {
					...
					try {
						...
						int width = 0;
						if (sheight != null)
							height = Integer.parseInt(parser.getAttributeValue(
									null, "height"));
						if (swidth != null)
							width = Integer.parseInt(parser.getAttributeValue(
									null, "width"));
