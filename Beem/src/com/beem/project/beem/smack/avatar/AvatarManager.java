...
/**
 * This class deals with the avatar data. It can be configured to auto retrieve
 * the avatar and put it in cache.
 * 
 */
public class AvatarManager {
	 ...
	 *            the connection
	 * @param pepMgr
	 *            the PepSubManager of the Connection
	 * @param cache
	 *            the cache which will store the avatars
	 * @param autoDownload
	 *            true to enable auto download of avatars
	 */
	 ...
	 *            true to enable auto download of avatars
	 */
	protected AvatarManager(final Connection con, final PepSubManager pepMgr,
	 ...
	 *            the id of the avatar
	 * @param info
	 *            the metadata information of the avatar to download
	 * @return true if the download was successfull
	 */
	public boolean downloadAvatar(String from, String avatarId, Info info) {
		try {
			AvatarRetriever retriever = AvatarRetrieverFactory.getRetriever(
					mCon, from, info);
	...
	public boolean publishAvatarData(byte[] data) {
		try {
			...
			System.err.println("Security error while publishing avatar data : "
					+ e.getMessage());
			return false;
		}
	}
	...
	/**
	 * Send the metadata of the avatar you want to publish. By sending this
	 * metadata, you publish an avatar.
	 * 
	 * @param id
	...
	public void publishAvatarMetaData(String id,
		...
		PayloadItem<AvatarMetadataExtension> item = new PayloadItem<AvatarMetadataExtension>(
				id, metadata);
	...
	/**
	 * Select the avatar to download. Subclass should override this method to
	 * take control over the selection process. This implementation select the
	 * first element.
	 * 
	 * @param available
	 *            list of the avatar metadata information
	 ...
	 *             if the sha-1 algorithm is unavailable
	 */
	...
	private void publishAvatarData(String id, byte[] data) {
		...
		PayloadItem<AvatarExtension> item = new PayloadItem<AvatarExtension>(
				id, avatar);
	 ...
	 *            the new avatar id
	 * @param avatarInfos
	 *            the metadata infos of the avatar
	 */
	private void fireListeners(String from, String avatarId,
