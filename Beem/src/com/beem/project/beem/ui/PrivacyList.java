...
public class PrivacyList extends ListActivity {
	...
	protected void onSaveInstanceState(Bundle savedInstanceState) {
		...
		savedInstanceState.putStringArrayList(SAVED_INSTANCE_KEY_PRIVACY_LISTS,
				(ArrayList<String>) mPrivacyListNames);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null && !savedInstanceState.isEmpty()) {
			mPrivacyListNames.addAll(savedInstanceState
					.getStringArrayList(SAVED_INSTANCE_KEY_PRIVACY_LISTS));
		}

		mAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_list_item_1, mPrivacyListNames);
	...
	protected void onDestroy() {
		...
		if (mPrivacyListManager != null) {
			try {
				mPrivacyListManager
						.removePrivacyListListener(mPrivacyListListener);
	...
	protected Dialog onCreateDialog(int id) {
		...
		switch (id) {
		case DIALOG_CREATE:
			dialog = new CreatePrivacyList(PrivacyList.this,
					mPrivacyListManager).create();
			...
			break;
		case DIALOG_DELETE:
			dialog = new DeletePrivacyList(PrivacyList.this,
					mPrivacyListManager, mCurrPrivacyListName).create();
			dialog.setOnDismissListener(new OnDismissListener() {
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		mCurrPrivacyListName = mPrivacyListNames
				.get(((AdapterView.AdapterContextMenuInfo) menuInfo).position);
	...
	private final class BeemServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				...
				mPrivacyListManager
						.addPrivacyListListener(mPrivacyListListener);
	...
	private class PrivacyListListener extends IPrivacyListListener.Stub {
		...
		@Override
		public void setPrivacyList(String listName,
				List<PrivacyListItem> listItem) throws RemoteException {
		...
		public void updatedPrivacyList(final String listName)
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					try {
						mPrivacyListNames.clear();
						// Not that much lists and require some server queries
						// to know if the list has been
						// updated/deleted or set to default/active by this
						// activity or another IM client.
						mPrivacyListNames.addAll(mPrivacyListManager
								.getPrivacyLists());
					} catch (RemoteException e) {
						Log.e(TAG, e.getMessage());
					}
