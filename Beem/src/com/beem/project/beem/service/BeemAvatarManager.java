...
/**
 * An AvatarManager for Beem. It allows to publish avatar on the Android
 * platform.
 */
public class BeemAvatarManager extends AvatarManager {
	 ...
	 * @param autoDownload
	 *            tre to enable auto download of avatars
	 */
	public BeemAvatarManager(final Context ctx, final Connection con,
			final PepSubManager pepMgr, final AvatarCache cache,
	...
	public boolean publishAvatar(Uri avatarUri) {
		try {
			Bitmap bmp = MediaStore.Images.Media.getBitmap(
					mContext.getContentResolver(), avatarUri);
	...
	/**
	 * Publish an avatar. This will send the XMPP stanza to enable the
	 * publication of an avatar.
	 * 
	 * @param bitmap
	 *            the avatar to publish
	...
	private boolean publishAvatar(Bitmap bitmap) {
		...
		AvatarMetadataExtension.Info jpeg = publishBitmap(bitmap,
				Bitmap.CompressFormat.JPEG, JPEG_QUALITY);
		// The png format is mandatory for interoperability
		AvatarMetadataExtension.Info png = publishBitmap(bitmap,
				Bitmap.CompressFormat.PNG, JPEG_QUALITY);
		if (png == null)
			return false;
		meta.addInfo(png);
		if (jpeg != null)
	 ...
	 *            the avatar bitmap
	 * @param format
	 *            the image format to publish this data
	 * @param quality
	 *            the compression quality use for JPEG compression
	 * @return the resulting info associate with this bitmap. null if the
	 *         operation failed
	 */
	private AvatarMetadataExtension.Info publishBitmap(Bitmap bmp,
	 ...
	 */
	private byte[] getBitmapByte(Bitmap bitmap, Bitmap.CompressFormat format,
			int quality) {
